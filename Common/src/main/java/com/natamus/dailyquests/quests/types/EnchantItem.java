...
public class EnchantItem extends AbstractQuest {
	...
	public boolean isAllowedIdentifier(Level level, ResourceLocation identifier) {
		...
		ItemStack itemStack = new ItemStack(itemRegistry.get(identifier));

		Optional<HolderSet.Named<Enchantment>> namedOptional = level.registryAccess().registryOrThrow(Registries.ENCHANTMENT).getTag(EnchantmentTags.IN_ENCHANTING_TABLE);
