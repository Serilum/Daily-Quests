...
public class UseItem extends AbstractQuest {
	...
	public boolean isEnabled() {
		return ConfigHandler.enableUseItemQuests;
	}
	...
	public boolean isAllowedIdentifier(Level level, ResourceLocation identifier) {
		return Variables.usableItems.contains(identifier);
	}

	@Override @Nullable
	public ResourceLocation getRandomQuestIdentifier(Level level) {
		return Variables.usableItems.get(Constants.random.nextInt(Variables.usableItems.size()));
	}

	@Override
	public int getRandomQuestProgressGoal(Level level, ResourceLocation identifier) {
		return Constants.random.nextInt(2, 12) + 1;
	}
