...
public class DailyQuestsClientEvents {
	...
	public static void renderOverlay(GuiGraphicsExtractor guiGraphics, @Nullable DeltaTracker deltaTracker, @Nullable PauseScreen pauseScreen) {
		...
		if (VariablesClient.questTitles.isEmpty()) {
			return;
		}
		...
		for (int n = 0; n < questCount; n++) {
			...
			if (!VariablesClient.questListCollapsed) {
				...
				guiGraphics.text(font, Component.literal(progressText), textXPosition, initialY + 1 + progressTextYOffset, 0xFFFFFFFF, ConfigHandler.questListDrawTextShadow);
			}
		}
		...
		for (QuestVarGUI questText : elementsToDraw) {
			if (questText.textComponent != null) {
				guiGraphics.text(font, questText.textComponent, questText.xPosition, questText.yPosition, questText.rgb, ConfigHandler.questListDrawTextShadow);

				if (!VariablesClient.playerDataObject.isShowingIntroduction()) {
					if (!questText.questProgressPair.getFirst().equals(questText.questProgressPair.getSecond())) {
						guiGraphics.text(font, "x", width - 15, questText.yPosition - 1, ConstantsClient.questProgressBarRedRGB, ConfigHandler.questListDrawTextShadow);
					} else {
						guiGraphics.text(font, "✔", width - 15, questText.yPosition - 1, ConstantsClient.questTitleRGB, ConfigHandler.questListDrawTextShadow);
					}
				...
				guiGraphics.text(font, questText.textSequence, questText.xPosition, questText.yPosition, questText.rgb, ConfigHandler.questListDrawTextShadow);
				...
				guiGraphics.text(font, Component.literal(progressText), questText.xPosition + (barWidth / 2) - (font.width(progressText) / 2), questText.yPosition+1, 0xFFFFFFFF, ConfigHandler.questListDrawTextShadow);
			}

			if (questText.scale != 0F) {
				matrixStack.scale(questText.scale, questText.scale);
			}
		}
