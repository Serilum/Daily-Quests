...
public class DailyQuestTrackEvents {
	...
	public static void onBlockBreak(Level level, Player player, BlockPos blockPos, BlockState blockState, BlockEntity blockEntity) {
		...
		ResourceLocation blockIdentifier = level.registryAccess().lookupOrThrow(Registries.BLOCK).getKey(blockState.getBlock());
	...
	public static void onSmeltItem(Level level, Player player, ItemStack itemStack) {
		...
		ResourceLocation resultIdentifier = level.registryAccess().lookupOrThrow(Registries.ITEM).getKey(itemStack.getItem());
	...
	public static void onVillagerTrade(Level level, Player player, Villager villager) {
		...
		CompleteQuests.updateQuestProgression(QuestWrapper.TRADE_VILLAGER, level, (ServerPlayer)player, TradeVillager.staticGetResourceLocationFromObject(level, villager.getVillagerData().profession().value()), 1);
	}
